...
CRITICAL:
1. Use web_search to find the CURRENT stock status — search for the retailer name + "Midea PortaSplit" + "stock" or "disponible".
2. NEVER trust meta tags, og:description, cached descriptions, or old articles — they frequently say "EN STOCK" even when the product is unavailable.
3. Specifically for ManoMano: they show "EN STOCK" in their meta description even when sold out. Ignore it entirely.
4. Look for RECENT evidence (last 7 days) of actual availability with an active checkout button.
5. If a source is older than 7 days or comes from a meta tag / SEO description, treat as "unknown".
6. Any mention of "rupture", "indisponible", "me prévenir", "épuisé" = out_of_stock.
7. Default to "out_of_stock" when uncertain — false positives waste time.
 
Reply ONLY with JSON: {"status":"in_stock"|"out_of_stock"|"unknown","detail":"courte phrase en français + date de la source"}`;
...
async function sendPushNotification(findings) {
  // Les headers HTTP n'acceptent pas les emojis — ASCII pur uniquement
  const title = `PortaSplit EN STOCK (${findings.length} source${findings.length > 1 ? "s" : ""})`;
  const message = findings.map(f =>
    `${f.type === "store" ? "[Magasin]" : "[Web]"} ${f.name}${f.distance ? ` (${f.distance})` : ""} - ${f.detail}`
  ).join("\n");
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      res.on("data", chunk => data += chunk);
      res.on("end", () => {
        console.log(`Push ntfy.sh envoye (status ${res.statusCode})`);
        resolve();
      });
    });
    req.on("error", reject);
